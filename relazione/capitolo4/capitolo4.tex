...
Il traffico verso il database viene intercettato ed ispezionato a livello L7 dal filtro di rete \texttt{mongo\_proxy} di Envoy. Quest'ultimo estrae i dettagli e i metadati della query di ricerca (\texttt{db\_query}) e li trasmette sincronicamente ad OPA. La policy di OPA (esaminata nel Capitolo 3) controlla il payload: se rileva parole chiave vietate (come \texttt{dropdatabase} o \texttt{deleteall}), imposta la variabile di blocco a \texttt{true}.
